-- ============================================================================
-- GENERAR 300 REGISTROS - SOLO CAMPOS REQUERIDOS
-- ============================================================================
-- Campos REQUERIDOS: id, fecha_solicitud, numero_solicitud, proveedor, concepto, 
-- monto_solicitud, total_solicitud, estado, created_at, updated_at
-- Campos OPCIONALES: iva y otros
-- ============================================================================

-- PASO 1: Ajustar tabla para campos opcionales
ALTER TABLE public.ordenes_pago ALTER COLUMN iva DROP NOT NULL;
ALTER TABLE public.ordenes_pago ALTER COLUMN iva SET DEFAULT NULL;

-- PASO 2: Corregir constraint de estados
...
-- PASO 3: Generar 300 registros con SOLO campos requeridos
DO $$
DECLARE
    i INTEGER;
...
BEGIN
    RAISE NOTICE 'Generando 300 registros con solo campos requeridos...';
    
    FOR i IN 1..300 LOOP
        -- Fecha aleatoria del último año
        fecha_solicitud := '2023-01-01'::date + floor(random() * 730)::integer;
        
        -- Monto base aleatorio entre 500,000 y 5,000,000
        monto_base := (500000 + floor(random() * 4500000))::NUMERIC;
        monto_base := ROUND(monto_base, -3); -- Redondear a miles
        
        -- Total solicitud (puede ser igual al monto base o incluir IVA calculado internamente)
        -- Para simplicidad, asumimos que total_solicitud incluye cualquier impuesto
        total_calc := ROUND(monto_base * 1.19, 2); -- Incluir IVA implícitamente en el total
        
        -- INSERTAR SOLO CAMPOS REQUERIDOS
        INSERT INTO public.ordenes_pago (
            fecha_solicitud,        -- REQUERIDO
            numero_solicitud,       -- REQUERIDO  
            proveedor,             -- REQUERIDO
            concepto,              -- REQUERIDO
            monto_solicitud,       -- REQUERIDO
            total_solicitud,       -- REQUERIDO
            estado,                -- REQUERIDO
            created_at,            -- REQUERIDO
            updated_at             -- REQUERIDO
            -- iva es OPCIONAL, no se incluye
        ) VALUES (
            fecha_solicitud,
...
-- PASO 4: Agregar registros adicionales en fechas específicas
DO $$
DECLARE
...
BEGIN
    ...
    FOR i IN 1..array_length(fechas_extra, 1) LOOP
        ...
        FOR j IN 1..(2 + floor(random() * 3)::integer) LOOP
            ...
            total_extra := ROUND(monto_extra * 1.19, 2);
...
SELECT 'VERIFICACIÓN DE CAMPOS REQUERIDOS:' as info;

-- Verificar que todos los campos requeridos tienen valores
SELECT 'CAMPOS NULOS EN REGISTROS:' as info;
SELECT 
    SUM(CASE WHEN fecha_solicitud IS NULL THEN 1 ELSE 0 END) as fecha_solicitud_nulos,
    SUM(CASE WHEN numero_solicitud IS NULL THEN 1 ELSE 0 END) as numero_solicitud_nulos,
    SUM(CASE WHEN proveedor IS NULL THEN 1 ELSE 0 END) as proveedor_nulos,
    SUM(CASE WHEN concepto IS NULL THEN 1 ELSE 0 END) as concepto_nulos,
    SUM(CASE WHEN monto_solicitud IS NULL THEN 1 ELSE 0 END) as monto_solicitud_nulos,
    SUM(CASE WHEN total_solicitud IS NULL THEN 1 ELSE 0 END) as total_solicitud_nulos,
    SUM(CASE WHEN estado IS NULL THEN 1 ELSE 0 END) as estado_nulos,
    ...
    -- Campo opcional
    SUM(CASE WHEN iva IS NULL THEN 1 ELSE 0 END) as iva_nulos_opcional
...
SELECT 'ESTADÍSTICAS DE MONTOS (SOLO CAMPOS REQUERIDOS):' as info;
SELECT 
    ...
    MIN(total_solicitud) as total_minimo,
    MAX(total_solicitud) as total_maximo,
    ROUND(AVG(total_solicitud), 0) as total_promedio,
    SUM(total_solicitud) as suma_total_solicitud
FROM public.ordenes_pago 
WHERE estado = 'Solicitada';

SELECT '🎉 GENERACIÓN COMPLETADA - SOLO CAMPOS REQUERIDOS 🎉' as resultado;
SELECT '✅ Todos los campos requeridos poblados correctamente' as detalle;
SELECT '✅ Campo iva es opcional (puede ser NULL)' as detalle;
